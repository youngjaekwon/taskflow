...
class TestPasswordResetConfirmView:
    def test_reset_confirm_success(self, api_client, verified_user):
        ...
        response = api_client.post(
            RESET_CONFIRM_URL,
            {"uid": uid, "token": token, "new_password": "ResetStrong789!"},
            format="json",
        )
        assert response.status_code == 200
    ...
    def test_reset_confirm_invalid_token(self, api_client, verified_user):
        ...
        response = api_client.post(
            RESET_CONFIRM_URL,
            {"uid": uid, "token": "bad-token", "new_password": "ResetStrong789!"},
            format="json",
        )
    ...
    def test_reset_confirm_invalid_uid(self, api_client):
        response = api_client.post(
            RESET_CONFIRM_URL,
            {"uid": "bad-uid", "token": "bad-token", "new_password": "ResetStrong789!"},
            format="json",
        )
        assert response.status_code == 400
