...
7. `.worktreeinclude` 파일 처리 (공유 파일 복사)
   ...
   - 있으면 파일을 읽어 패턴 목록을 파싱한다:
     ...
     - 각 줄을 하나의 패턴으로 취급한다
   - 각 패턴을 다음 규칙으로 glob으로 변환한다:
     - 디렉터리 패턴(trailing `/`)은 `/`를 제거한다 (예: `.vscode/` → `.vscode`)
     - 모든 패턴에 `**/` 접두어를 붙인다 (예: `db.sqlite3` → `**/db.sqlite3`, `.env.*.local` → `**/.env.*.local`)
     - glob 와일드카드 `*`, `?`, `**`를 지원한다
   - 변환된 glob으로 레포 루트를 기준으로 재귀적으로 매칭되는 파일/디렉터리를 찾는다
   - 매칭된 항목을 워크트리 경로의 동일 상대 위치로 복사한다:
     - 파일: `cp` 명령으로 복사한다. 필요시 상위 디렉터리를 먼저 생성한다
     - 디렉터리 (원본 패턴이 `/`로 끝나거나 매칭 결과가 디렉터리인 경우): `cp -r` 명령으로 복사한다
